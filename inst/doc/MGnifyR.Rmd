...
`MGnifyR` is a package designed to ease access to the EBI's [MGnify](https://www.ebi.ac.uk/metagenomics) resource, allowing  easy searching and retrieval of multiple datasets for downstream analysis. While MGnify pipelines are undoubtedly useful, as currently implemented the pipelines produce results on a strictly per-sample basis. While whole study results are available, comparisons across studies are difficult. The MGnifyR package is specifically designed to facilitate cross-study analyses by handling all the per-sample merging details internally, leaving the user free to perform the analysis as they see fit.

## Installation

`MGnifyR` is hosted on github, and may be installed using via `devtools`. As things stand `MGnifyR` has dependencies on `ape`, `dplyr`, `plyr`, `httr`, `urltools` and `reshape2`, which should be installed automatically if missing. The `phyloseq` package is also required and should be installed according to the instructions at (https://joey711.github.io/phyloseq/install.html). If following through this vignette, `ggplot2` and `Biostrings` will also be required, but aren't essential for using MGnifyR (and will be brought in by installing `phyloseq` anyway.)

Once pacakge dependencies are installed, `MGnifyR` should be built using the following snippet.
...
Clearly all the files could first be downloaded in the standard way, and then processed using this same function in a loop. Therefore in many cases the `read_func` parameter is redundant. However, many of the outputs from MGnify can be quite large, meaning local storage of many files may become an issue. By providing a `read_func` parameter (and necessarily setting `usecache=F`) analysis of a large number of datasets may be possible with minimal storage requirements.
